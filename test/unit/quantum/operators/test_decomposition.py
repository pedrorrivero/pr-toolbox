...
"""Tests for quantum operators decomposition tools."""
...
from pr_toolbox.quantum.operators.decomposition import (
    AbelianDecomposer,
    NaiveDecomposer,
    OperatorDecomposer,
)
...
class TestOperatorDecomposer:
    ...
    @mark.parametrize(
        ...
        [
            ...
            (
                PauliList(["IXYZ", "ZYXI", "IXII", "ZYII"]),
                SparsePauliOp(["IXYZ", "ZYXI", "IXII", "ZYII"]),
            ),
        ],
    )
    def test_normalization(self, decomposer, unnormalized, normalized):
        """Test that input operators are normalized appropriately."""
        assert decomposer.decompose(unnormalized) == decomposer.decompose(normalized)
