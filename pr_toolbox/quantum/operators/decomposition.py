...
"""Quantum operators decomposition tools."""
...
class OperatorDecomposer(ABC):
    ...
    def decompose(self, operator: BaseOperator | PauliSumOp | str) -> tuple[SparsePauliOp, ...]:
        ...
        Note that component decomposition is not unique, for instance,
        commuting components could be grouped together in different ways
        (i.e. partitioning the set).

        Args:
            operator: the operator to decompose into its core components.

        Returns:
            A tuple of operators each of which measurable with a single
            quantum circuit (i.e. on a singlet Pauli basis).
        """
    ...
    def extract_pauli_bases(self, operator: BaseOperator | PauliSumOp | str) -> PauliList:
        ...
        Note that the resulting basis may be over-complete depending on the
        implementation.

        Args:
            operator: for which to obtain a Pauli measurement basis.

        Returns:
            A `PauliList` of operators serving as a measurement basis for
            the input operator. Each entry corresponds one-to-one to the
            components retrieved from `.decompose()`.
        """
    ...
    def _extract_singlet_pauli_basis(self, operator: SparsePauliOp) -> Pauli:
        ...
        The input operator comes from `._decompose()`, and must be singly
        measurable.
        """
        raise NotImplementedError  # pragma: no cover
