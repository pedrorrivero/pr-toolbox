...
This module provides support for strategy classes, which are similar in
format and spirit to those produced by the builtin :class:`dataclass`
decorator. In this case, the relevant information is extracted from the
class' ``__init__`` method signature rather than from class fields. Said
class signature specifies the so called 'settings' for the strategy, where
arguments whose name begins with a leading ``_`` are excluded from the
settings namespace. Although not currently enforced, it is strongly
suggested for these excluded arguments to be optional (i.e. developers
should provide defaults).

Init's arguments are referred to as 'init args', and are not subject to
any namespace exclusions. Similarly, construction-time init args (including
defaults) are referred to as 'original args': they are merely a reflection
of the values of all arguments passed to ``__init__`` when instantiating the
given strategy object.

Currently, init arguments of kind POSITIONAL_ONLY, VAR_POSITIONAL, and
VAR_KEYWORD are disallowed. See
https://docs.python.org/3/library/inspect.html#inspect.Parameter.kind for
more information.

Strategy classes include the following specific methods and fields:
    01. ``__init_subclass__``: to handle subclassing.
    02. ``__new__``: to save the original args at construction-time.
    03. ``__getattr__``: to retrieve original args via ``getattr``.
    ...
    09. ``settings``: a dictionary of settings and their corresponding values.
    10. ``init_args``: a dictionary of init args and their corresponding values.
    11. ``original_args``: a copy of the construction-time init args.
    12. ``is_facade``: to check if current class is a facade of a given ancestor.
    13. ``replicate``: to build replicas of strategy instances updating settings.

Out of these, {01, 02, 03} can extend and be extended, and {04, 05} can be
overridden; all others should be left unmodified.
...
Generally speaking, strategies are mutable, meaning that their settings can
be updated at runtime. The implicit assumption made by this decorator is
that the current state of all settings listed in ``SETTINGS_NAMESPACE`` can
be accessed through a ``getattr`` call. If any of these settings is not
accessible that way the corresponding (construction-time) init arg will be
retrieved instead, possibly leading to conflicts and runtime errors if
mutated after instantiation. If certain settings cannot be accessed by
design, the suggested solution is removing said settings from the settings
namespace as explained before, or adding a dummy property that retrieves
their state (i.e. getter).

Oftentimes, developers will need their strategies to be immutable after
instantiation. To this end, the :class:`~zne.utils.strategy.strategy`
decorator class accepts an optional keyword argument ``frozen`` which, if
set to ``True`` (default ``False``), will forbid all ``setattr`` calls, and
automatically return deep copies of mutable settings. In this scenario,
settings will still be declared through the class' ``__init__`` method
signature, while the body could be left blank (i.e. ``pass``), hold the
appropriate docstring, or used for validation among other things.

Replicas of any strategy instance can be produced by calling their
:method:`~zne.utils.strategy.strategy.replicate` method with updated
settings as keyword arguments. This is especially useful when dealing with
frozen (i.e. immutable) strategies.
"""
...
from pr_toolbox.descriptors.classconstant import classconstant
...
class strategy:  # pylint: disable=invalid-name
    ...
    Applies a strategy base class to the decorated class.

    This is done by dynamically subclassing from both the corresponding
    strategy base class (i.e. mutable or frozen), and the pre-decorated class
    as declared by the developer. The base strategy class will have precedence
    in the resulting class' mro; therefore, the resulting strategy class will
    extend the ``__init_subclass__``, ``__new__``, and ``__getattr__`` methods
    from the pre-decorated class. Additionally, in order to allow redefinition
    of the base strategy class' ``__init__`` (dummy), ``__repr__`` and
    ``__eq__`` methods, if these are found in the pre-decorated class'
    dictionary, they will be bridged to the resulting class; effectively
    overriding the defaults. Similarly, ``__module__``, ``__name__``,
    ``__annotations__``, and ``__doc__``, will also be bridged from the
    pre-decorated class as is standard
    in decorators.
